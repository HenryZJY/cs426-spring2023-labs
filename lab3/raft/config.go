...
func (cfg *config) nCommitted(index int) (int, interface{}) {
	...
	for i := 0; i < len(cfg.rafts); i++ {
		...
		// cfg.t.Logf("logs[%v][%v] = %v, %v", i, index, cmd1, ok)
		cfg.mu.Unlock()
	...
	for time.Since(t0).Seconds() < 25 {
		...
		if index != -1 {
			...
			for time.Since(t1).Seconds() < 5 {
				...
				// cfg.t.Logf("one(%v) found %v/%v servers committed cmd:%v, index is %v", cmd, nd, expectedServers, cmd1,  index)
			...
			if !retry {
				cfg.t.Fatalf("one(%v) failed to reach agreement case 1", cmd)
			}
		} else {
			time.Sleep(50 * time.Millisecond)
		}
	}
	cfg.t.Fatalf("one(%v) failed to reach agreement case 2", cmd)
	return -1
}
